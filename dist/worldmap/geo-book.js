...
import { PATH_ALIASES, SEGMENT_ALIASES, tierOf } from './graph.js';
/**
 ...
 */
export function selectCoordNodes(graph, options) {
 ...
 */
export function keysForNode(node) {
    const keys = new Set();
    if (node.name)
        keys.add(node.name);
 ...
 */
export function buildCoordDrafts(graph, options = {}) {
...
export function draftToEntry(draft, uid) {
    return {
        uid,
        name: draft.name,
        enabled: true,
...
export function buildWorldbookEntries(graph, options = {}) {
...
export function entriesDiffer(existing, drafts) {
...
export function toSillyTavernBook(drafts) {
    const entries = {};
...
export function detectGeoBookCapabilities() {
    const notes = [];
 ...
 *   · 书里有别人的条目 → 拒绝覆盖并报出条目名。
 */
export async function syncCoordBook(graph, options) {
...
/** 当前挂载状态（设置页状态徽章的数据源） */
export function readCoordMountState() {
    const caps = detectGeoBookCapabilities();
    if (!caps.canAttach) {
 ...
 */
export async function attachCoordBook() {
...
export async function detachCoordBook() {
    const current = getCharWorldbookNames('current');
    const primary = current.primary ?? null;
...
export async function deleteCoordBook() {
    try {
