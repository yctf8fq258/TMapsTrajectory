...
export function stripEntryPrefix(raw) {
 ...
 */
export function cutDescription(raw) {
...
export function nodeId(parentId, name) {
...
export function isBearingSegment(segment) {
...
export function isImmortalRealmName(name) {
...
export function isTrivialFacility(name) {
 ...
 * 都能被下面几条规则命中。
 */
export function looksLikeDescription(segment) {
    if (segment.length >= 9)
        return true;
    if (/[，。；！？、]/.test(segment))
        return true;
...
export function parseBearing(text) {
    const dirs = [];
