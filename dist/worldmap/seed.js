 ...
 */
export function offsetFromGodCapital(distanceYi, degrees, bearing) {
...
export const SEED_NODES = expand(rows, 'seed');
/** 神都内部细节：世界书《洛阳扩展》里的同心四区 */
export const SEED_CAPITAL_NODES = expand(godCapitalRows, 'seed');
export const SEED_ALL = [...SEED_NODES, ...SEED_CAPITAL_NODES].filter((node, index, list) => list.findIndex(item => item.id === node.id) === index);
