/**
 * 视觉：深色应用外壳（暖炭色 + 米黄亚麻强调）× 宣纸古地图画布。
 * 设计令牌集中在 .worldmap-root 的 CSS 变量里，想调气质只改变量即可。
...
export const UI: Record<'layers' | 'pin' | 'pencil' | 'route' | 'sliders' | 'panelRight' | 'locate' | 'fit' | 'close' | 'chevLeft' | 'chevRight' | 'plus' | 'minus' | 'eye', IconShape> = {
  layers: {
    paths: [
...
export function svgIcon(icon: IconShape, size = 16, strokeWidth = 2): string {
