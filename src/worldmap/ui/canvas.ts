/**
 * SVG 画布：单一全局坐标系 + 相机（平移/缩放）+ 高德式分级显示 + 轨迹 + 拖拽编辑。
 *
 * 显示规则（对应"像高德地图那样"）：
 *   · 每个节点按 kind 归入 1~5 级（宗级势力与城池同级），级别越低（越大）越早出现；
 *   · 缩放小于该级别的阈值就不画点，再小就不画标签；
 *   · 屏幕上彼此距离 < COLLAPSE_PX 的点只画优先级最高的一个 —— 所以缩小时洛阳是一颗点，
 *     放大之后宫城区/四区/六坊/环墟才各自散开；
 *   · 垂直重叠（广寒宫 / 北冥黑渊）把下方那个往旁边挪一段屏幕距离，并用虚线连回原位。
 */
...
export interface CanvasView {
  ...
  /** 轨迹只画到市级（tier ≤ 3）：避免屋里走动画成蜘蛛网 */
  trailCityOnly: boolean;
  /** null = 显示全部轨迹；数字 = 只显示到该序号为止 */
...
function padPolygon(polygon: Vec2[], margin: number): Vec2[] {
  ...
  return polygon.map(([x, y]) => {
    ...
    return [x + (dx / len) * margin, y + (dy / len) * margin] as Vec2;
  });
}
...
export interface CanvasHooks {
  ...
  onMoveNode: (id: string, xy: Vec2) => void;
  /** 框选批量拖动结束：一次性提交整组新坐标 */
  onMoveNodes: (items: { id: string; xy: Vec2 }[]) => void;
  /** 拖拽开始时打撤销快照（必须在坐标被实时改写之前，否则撤销等于没撤） */
  onMoveSnapshot?: () => void;
  /** 拖拽结束但没动：把开拖时打的快照退回去 */
  onMoveAborted?: () => void;
  /** 框选结束：选中数量反馈（0 不回调） */
  onBoxSelected?: (count: number) => void;
  onEditNode: (id: string) => void;
  /** 编辑模式下在空白处右键 / 双击：在指定坐标新建节点 */
  onCreateNodeAt: (xy: Vec2, parentId: string | null) => void;
}
...
export class MapCanvas {
  ...
  private regionLayer: SVGGElement;
  ...
  private drag: {
    ...
    originY: number;
    moved: boolean;
    /** group 模式：整组选中点的拖动起点（世界坐标） */
    origins?: Map<string, Vec2>;
  } | null = null;
  private suppressClick = false;
  private offsets = new Map<string, Vec2>();
  ...
  /** 框选模式开关 */
  setBoxSelect(value: boolean): void {
  ...
  setView(partial: Partial<CanvasView>): void {
  ...
  fit(ids?: string[]): void {
    ...
    const nodes = ids?.length
      ? (ids.map(id => this.view.graph.get(id)).filter(Boolean) as MapNode[])
  ...
  fitStage(): void {
    ...
    let stage: MapNode | undefined = chain[0];
    if (!stage) stage = this.view.graph.byPath.get('中央神州');
    if (!stage) {
    ...
    if (stage) this.focusOn(stage.id);
    else this.fit();
  }
  ...
  private computeOffsets(): void {
    ...
    for (const node of this.view.graph.toArray()) {
      if (this.view.hiddenNodeIds.has(node.id)) continue;
    ...
    for (const [, group] of groups) {
      ...
      alts.forEach((node, index) => {
        ...
        const pos: Vec2 = [node.xy[0] + side * pixel * rank, node.xy[1] + (index % 2 === 1 ? pixel * 0.42 : 0)];
        this.offsets.set(node.id, pos);
        this.connectors.push({ from: base.xy, to: pos });
      });
      for (const node of group) if (!this.offsets.has(node.id)) this.offsets.set(node.id, node.xy);
    }
  }

  /** 节点的实际绘制坐标（含高度偏移） */
  worldPos(node: MapNode): Vec2 {
  ...
  private currentLocationId(): string | null {
    if (!this.view.currentPath) return null;
    return this.view.graph.byPath.get(this.view.currentPath)?.id ?? null;
  }

  private priority(node: MapNode, currentId: string | null): number {
    let score = 100 - tierOf(node) * 12;
    if (node.id === currentId) score += 800;
    if (node.id === this.view.selectedId) score += 1000;
    if (node.locked) score += 0.5;
  ...
  render(): void {
    ...
    for (const node of graph.toArray()) this.childCount.set(node.id, graph.children(node.id).length);
    this.computeOffsets();
    ...
    const addChain = (id: string | null) => {
      for (const node of graph.ancestors(id)) keep.add(node.id);
      if (id) keep.add(id);
    };
    addChain(this.view.focusId);
    addChain(currentId);
    addChain(this.view.selectedId);
    for (const id of trailNodeIds) addChain(id);
    ...
    for (const node of ranked) {
      ...
      const tooClose = accepted.some(
        item => Math.abs(item.sx - sx) < COLLAPSE_PX && Math.abs(item.sy - sy) < COLLAPSE_PX * 0.6,
      );
    ...
    if (this.view.showRegions) {
      const childMap = new Map<string, MapNode[]>();
      for (const node of graph.toArray()) {
        if (!node.parentId) continue;
        const list = childMap.get(node.parentId);
        if (list) list.push(node);
        else childMap.set(node.parentId, [node]);
      }
      for (const node of candidates) {
        if (tierOf(node) > 2 || node.status === 'unplaced') continue;
        const points: Vec2[] = [];
        const stack = [node.id];
        ...
        while (stack.length) {
          ...
          if (self && self.status === 'ok') points.push(self.xy);
        ...
        const polygon =
          node.shape && node.shape.length >= 3 ? node.shape : padPolygon(convexHull(points), REGION_PAD);
    ...
    if (this.view.showLinks && linkFade > 0.04) {
      ...
      for (const item of accepted) {
        ...
        if (!parentId || !acceptedIds.has(parentId)) continue;
    ...
    const labelCandidates: { node: MapNode; pos: Vec2; tier: number }[] = [];
    ...
    for (const item of accepted) {
      ...
      for (const other of accepted) {
        ...
        if (Math.abs(other.sx - item.sx) < 64 && Math.abs(other.sy - item.sy) < 52) near++;
      }
    ...
    for (const d of COMPASS.paths) compass.appendChild(el('path', { d }));
    for (const [cx, cy, r] of COMPASS.circles) compass.appendChild(el('circle', { cx, cy, r }));
  ...
  private finishRubber(): void {
    if (this.rubber) {
      ...
      for (const node of this.view.graph.toArray()) {
        if (this.view.hiddenNodeIds.has(node.id)) continue;
        if (node.status === 'unplaced' && !this.view.showUnplaced) continue;
        if (node.pinned) continue; // 位置固定：框选直接跳过
        if (this.scale < (TIER_VISIBLE_SCALE[tierOf(node)] ?? 0)) continue;
        const pos = this.worldPos(node);
        if (pos[0] >= wx0 && pos[0] <= wx1 && pos[1] >= wy0 && pos[1] <= wy1) picked.add(node.id);
      }
      this.multi = picked;
      if (picked.size) this.hooks.onBoxSelected?.(picked.size);
    }
    this.rubber = null;
    this.render();
  }
  ...
  private hitTest(clientX: number, clientY: number): MapNode | null {
  ...
  private hitTestAll(clientX: number, clientY: number): MapNode[] {
    const [wx, wy] = this.screenToWorld(clientX, clientY);
    const hits: { node: MapNode; d: number }[] = [];
    for (const node of this.drawn) {
      const tier = tierOf(node);
      const radiusPx = TIER_RADIUS_PX[Math.min(tier, TIER_RADIUS_PX.length - 1)] ?? 6;
      const pos = this.worldPos(node);
      ...
      const dy = pos[1] - wy;
      const distance = Math.sqrt(dx * dx + dy * dy);
      if (distance < threshold) hits.push({ node, d: distance });
    }
   ...
   */
  private lastPick: { x: number; y: number; ids: string[]; index: number } | null = null;
  private pickedAtDown: MapNode | null = null;

  private pickNode(clientX: number, clientY: number): MapNode | null {
  ...
  private bind(): void {
    ...
    this.svg.addEventListener('pointerdown', event => {
      ...
      if (this.view.editMode || this.boxSelectMode) event.preventDefault();
      ...
      if (this.view.editMode) {
        ...
        if (node && shift) {
          // Shift + 点 = 加入/移出多选（不拖动）
          if (this.multi.has(node.id)) this.multi.delete(node.id);
          else this.multi.add(node.id);
          this.render();
          return;
        }
        if (node && !shift && this.multi.has(node.id) && this.multi.size > 1) {
          ...
          for (const id of this.multi) {
            ...
            if (picked && !picked.pinned) origins.set(id, this.worldPos(picked));
          }
          this.drag = {
            mode: 'group',
            startX: event.clientX,
...
export function defaultView(graph: MapGraph): CanvasView {
  return {
    graph,
    trail: [],
...
export function html(text: unknown): string {
