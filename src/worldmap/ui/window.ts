 ...
 */
import type { BaseMap, DrawerTab, LayoutState, MapNode, MapSettings, NodeSource, TrailPoint, Vec2 } from '../types.js';
...
export interface WindowActions {
  ...
  /** 人工改来源（trail ↔ manual/ai/…，跨层） */
  onSetNodeSource: (id: string, source: NodeSource) => void;
  /** 人工改路径（节点连同子树改挂层级） */
  onSetNodePath: (id: string, path: string) => void;
  ...
  onRebuildTrail: () => void;
  onClearHiddenPoints: () => void;
  onRunLayout: (scope: 'world' | 'region' | 'all') => void;
  onClearNodes: () => void;
  onTestApi: () => void;
  onPullPreset: () => void;
  onResetBaseMap: () => void;
...
export class MapWindow {
  ...
  private panes = new Map<DrawerTab, HTMLElement>();
  private tabButtons = new Map<DrawerTab, HTMLButtonElement>();
  ...
  private lastPathListFor?: string | null;
   ...
   */
  private host: Window;
  private dockSide: 'left' | 'right' | null = null;
  /** 贴边判定阈值（像素） */
  private readonly edgeThreshold = 18;

  constructor(private doc: Document, data: WindowData, private actions: WindowActions, private canvasWrap: HTMLElement) {
    this.host = (doc.defaultView ?? window) as Window;
    this.data = data;
    ensureStyle(doc);
  ...
  private build(): void {
    ...
    if (!canvasWrap.id) canvasWrap.id = `${ID_PREFIX}canvas`;
  ...
  private buildPanes(): void {
    ...
    layers.querySelectorAll<HTMLInputElement>('[data-layer]').forEach(input => {
      input.addEventListener('change', () =>
        this.actions.onToggleLayer(input.dataset.layer as 'showTrail' | 'showLinks' | 'showRegions' | 'showUnplaced' | 'trailCityOnly', input.checked),
      );
    });
    layers.querySelector('[data-act=scatter]')?.addEventListener('click', () => this.actions.onScatterUnplaced());

    // ── 地点 ──
    const places = this.panes.get('places') as HTMLElement;
    places.innerHTML = `
    ...
    this.searchInput = places.querySelector('[data-role=search]') as HTMLInputElement;
    ...
    const applyXy = () => {
      const x = Number((edit.querySelector('[data-role=node-x]') as HTMLInputElement).value);
      const y = Number((edit.querySelector('[data-role=node-y]') as HTMLInputElement).value);
    ...
    edit.querySelector('[data-act=add-child]')?.addEventListener('click', () => {
      ...
      if (this.data.selectedId) this.actions.onAddChild(this.data.selectedId, name);
    });
    edit.querySelector('[data-act=add-sibling]')?.addEventListener('click', () => {
      ...
      if (selected) this.actions.onAddChild(selected.parentId, name);
    });
    ...
    const deleteButton = edit.querySelector('[data-act=delete-node]') as HTMLButtonElement;
    ...
    (edit.querySelector('[data-role=node-name]') as HTMLInputElement | null)?.addEventListener('change', event => {
      if (this.data.selectedId) this.actions.onRenameNode(this.data.selectedId, (event.target as HTMLInputElement).value);
    });
    // 路径编辑：改挂层级（回车/失焦生效）
    const pathInput = edit.querySelector('[data-role=node-path]') as HTMLInputElement | null;
    pathInput?.addEventListener('change', () => {
      if (this.data.selectedId) this.actions.onSetNodePath(this.data.selectedId, pathInput.value);
    });
    // 来源编辑：跨层移动（trail ↔ 底图层）
    const sourceSelect = edit.querySelector('[data-role=node-source]') as HTMLSelectElement | null;
    sourceSelect?.addEventListener('change', event => {
      if (this.data.selectedId) {
        this.actions.onSetNodeSource(this.data.selectedId, (event.target as HTMLSelectElement).value as NodeSource);
      }
    });
    ...
    settings.querySelector('[data-act=toggle-key]')?.addEventListener('click', () => {
      const input = settings.querySelector('[data-set=key]') as HTMLInputElement;
      input.type = input.type === 'password' ? 'text' : 'password';
    });
    (settings.querySelector('[data-role=model-select]') as HTMLSelectElement | null)?.addEventListener('change', event => {
      const value = (event.target as HTMLSelectElement).value;
      if (!value) return;
      const input = settings.querySelector('[data-set=model]') as HTMLInputElement;
    ...
    const clearButton = settings.querySelector('[data-act=clear-nodes]') as HTMLButtonElement;
    ...
    settings.querySelector('[data-act=export-anchors]')?.addEventListener('click', () => {
      this.setIo(
        this.actions.onExportAnchorText(),
        '这是可以直接替换提示词里「固定锚点」那一段的文本（照抄即可，坐标单位与地图一致）。',
      );
    });
  ...
  private ioPane(): HTMLElement {
    return this.panes.get('settings') as HTMLElement;
  }

  private ioValue(): string {
    return (this.ioPane().querySelector('[data-role=io]') as HTMLTextAreaElement)?.value ?? '';
  }

  private setIo(text: string, hint?: string): void {
    const area = this.ioPane().querySelector('[data-role=io]') as HTMLTextAreaElement | null;
    if (area) {
      area.value = text;
      area.focus();
      area.setSelectionRange(0, 0);
    }
    if (hint) this.setIoHint(hint);
  }

  private setIoHint(text: string): void {
    const hint = this.ioPane().querySelector('[data-role=io-hint]') as HTMLElement | null;
   ...
   */
  showReport(text: string, hint: string): void {
    const pane = this.ioPane();
    const report = pane.querySelector('[data-role=layout-report]') as HTMLElement | null;
    if (report) {
      report.textContent = text;
      const section = report.closest('details') as HTMLDetailsElement | null;
      if (section) section.open = true;
      report.scrollIntoView({ block: 'nearest' });
    }
    const area = pane.querySelector('[data-role=io]') as HTMLTextAreaElement | null;
  ...
  fillModels(models: string[]): void {
    const pane = this.panes.get('settings') as HTMLElement;
    const select = pane.querySelector('[data-role=model-select]') as HTMLSelectElement | null;
    if (!select) return;
    select.innerHTML =
      `<option value="">（共 ${models.length} 个，点这里选）</option>` +
      models.map(id => `<option value="${escapeHtml(id)}">${escapeHtml(id)}</option>`).join('');
    const current = (pane.querySelector('[data-set=model]') as HTMLInputElement)?.value;
  ...
  async fetchModels(): Promise<void> {
    const pane = this.panes.get('settings') as HTMLElement;
    const button = pane.querySelector('[data-act=fetch-models]') as HTMLButtonElement;
    if (button) {
    ...
    try {
      ...
      const urlInput = pane.querySelector('[data-set=url]') as HTMLInputElement | null;
      const keyInput = pane.querySelector('[data-set=key]') as HTMLInputElement | null;
  ...
  private async listModels(base: string, key: string): Promise<string[]> {
  ...
  /** 失败时补一句人话：接口到底通不通 */
  private async describeEndpoint(base: string): Promise<string> {
    try {
      const response = await fetch(`${base}/models`, { method: 'GET' });
      if (response.ok) return '';
      const body = await response.text().catch(() => '');
      if (response.status === 401 || response.status === 403) return '（密钥可能不对或没填）';
  ...
  private collectSettings(): void {
    const pane = this.panes.get('settings') as HTMLElement;
    const read = (key: string) => (pane.querySelector(`[data-set=${key}]`) as HTMLInputElement)?.value ?? '';
    this.actions.onSaveSettings({
      api: {
  ...
  private bindChrome(): void {
    ...
    bar.addEventListener('pointermove', event => {
      ...
      if (this.isRail() && !dragging.undocked) {
        if (Math.abs(dx) + Math.abs(dy) < 8) return; // 还没拖动，先当作待判定点击
    ...
    // 右下角缩放
    const handle = this.root.querySelector('.dym-resize') as HTMLElement;
    let resizing: { x: number; y: number; w: number; h: number } | null = null;
    handle.addEventListener('pointerdown', event => {
      if (this.isRail()) this.exitRail();
  ...
  private enterRail(side: 'left' | 'right'): void {
    this.dockSide = side;
    const rect = this.root.getBoundingClientRect();
  ...
  private currentTab: DrawerTab = 'places';
  ...
  applyLayout(layout: LayoutState): void {
    ...
    if (this.host.innerWidth < 720) this.root.classList.add('dym-narrow');
    // 最小化按钮已移除：即便旧布局里存着 collapsed，也强制展开，避免出现"打不开"的窗口
    this.root.classList.remove('dym-collapsed');
    if (layout.collapsed) this.data.layout.collapsed = false;
    this.setTab(layout.tab ?? 'places');
    this.toggleDrawer(layout.drawerOpen === false);
    if (layout.docked) this.enterRail(layout.docked);
  }

  setTab(tab: DrawerTab): void {
    this.currentTab = tab;
    for (const [key, button] of this.tabButtons) button.classList.toggle('dym-active', key === tab);
    for (const [key, pane] of this.panes) pane.classList.toggle('dym-active', key === tab);
  }
  ...
  toggleCollapsed(): void {
    ...
    this.persistLayout();
    if (!collapsed) this.data.canvas.render();
  }
  ...
  render(data: Partial<WindowData>): void {
    ...
    this.breadcrumb.querySelectorAll<HTMLElement>('[data-focus]').forEach(item => {
    ...
    const layers = this.panes.get('layers') as HTMLElement;
    layers.querySelectorAll<HTMLInputElement>('[data-layer]').forEach(input => {
      const key = input.dataset.layer as 'showTrail' | 'showLinks' | 'showRegions' | 'showUnplaced' | 'trailCityOnly';
      input.checked = Boolean(view[key]);
    });
    const unplaced = canvas.getView().graph.toArray().filter(node => node.status === 'unplaced').length;
    const hint = layers.querySelector('[data-role=layer-hint]') as HTMLElement;
    if (hint) hint.textContent = `节点 ${canvas.getView().graph.size} 个，其中待定位 ${unplaced} 个。待定位节点由轨迹自动落点产生，确认位置后可拖动微调。`;

    // 选中信息
    const edit = this.panes.get('edit') as HTMLElement;
    const selected = this.data.selectedId ? canvas.getView().graph.get(this.data.selectedId) : undefined;
    (edit.querySelector('[data-role=selected-info]') as HTMLElement).textContent = selected
    ...
    const pathInput = edit.querySelector('[data-role=node-path]') as HTMLInputElement | null;
    const sourceSelect = edit.querySelector('[data-role=node-source]') as HTMLSelectElement | null;
    if (pathInput && document.activeElement !== pathInput) pathInput.value = selected?.path ?? '';
    if (pathInput) pathInput.disabled = !selected;
    if (sourceSelect && document.activeElement !== sourceSelect) sourceSelect.value = selected?.source ?? 'trail';
    if (sourceSelect) sourceSelect.disabled = !selected;
    // 路径建议列表：选中节点变化时重建一次
    ...
    // 固定位置按钮：跟随选中点的状态
    const pinButton = edit.querySelector('[data-act=toggle-pin]') as HTMLButtonElement | null;
    if (pinButton) {
      const selected = this.data.selectedId ? this.data.canvas.getView().graph.get(this.data.selectedId) : undefined;
    ...
    (edit.querySelector('[data-act=redo]') as HTMLButtonElement).disabled = !this.data.canRedo;

    // 轨迹
    const trailPane = this.panes.get('trail') as HTMLElement;
    const trailHint = trailPane.querySelector('[data-role=trail-hint]') as HTMLElement;
    ...
    trailList.innerHTML = ordered
      ...
      .join('');
    trailList.querySelectorAll<HTMLElement>('[data-point]').forEach(item => {
  ...
  private renderPlaceList(): void {
    const list = (this.panes.get('places') as HTMLElement).querySelector('[data-role=place-list]') as HTMLElement;
    ...
    list.querySelectorAll<HTMLElement>('[data-node]').forEach(item => {
      const id = item.dataset.node as string;
