...
export class MapGraph {
   ...
   */
  moveNode(id: string, segments: string[]): boolean {
    const node = this.byId.get(id);
    if (!node || !segments.length) return false;
    ...
    for (const raw of segments.slice(0, -1)) {
      ...
      if (!seg) continue;
      const found: MapNode | undefined =
        this.findChild(parent?.id ?? null, seg) ?? this.findDescendantByName(parent?.id ?? null, seg, 2);
      let child: MapNode;
      if (found) child = found;
      else child = this.create(parent, seg, parent ? `${parent.path}·${seg}` : seg, 'manual');
      parent = child;
    }
    const newPath = parent ? `${parent.path}·${name}` : name;
    if (newPath === node.path) return false;
    const occupant = this.byPath.get(newPath);
    if (occupant && occupant.id !== id) return false;
    // 防环：新父链不能包含自己
    for (let cursor: MapNode | null = parent; cursor; cursor = this.get(cursor.parentId) ?? null) {
    ...
    let queue: MapNode[] = [...this.children(id)];
    while (queue.length) {
      const next: MapNode[] = [];
